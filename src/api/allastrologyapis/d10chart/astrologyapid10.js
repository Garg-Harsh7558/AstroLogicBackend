...
/**
 * Logic to fetch Dasamsha Chart Info (D10 Chart)
 */
export const fetchD10Chart = async (argumentoncall) => {
    const {
        year = 2002,
        month = 8,
        date = 11,
    ...
    const options = {
        method: 'POST',
        url: 'https://json.freeastrologyapi.com/d10-chart-info',
        headers: {
 ...
 */
const getD10ChartInfoController = async (req, res) => {
    try {
        const datafromreq = Object.keys(req.body || {}).length > 0 ? req.body : req.query;
        const data = await fetchD10Chart(datafromreq);
        
        return res.status(200).json({
            success: true,
            d10Chart: data.output
        });
    } catch (error) {
        console.error("D10 Chart Info Error:", error.response?.data || error.message);
        return res.status(error.response?.status || 500).json({
            success: false,
            message: "Failed to fetch D10 chart info",
            error: error.response?.data || error.message
        });
    }
};

export default getD10ChartInfoController;
